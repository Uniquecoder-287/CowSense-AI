          ...
          {mockNotifications.map((notif, index) => (
            <motion.div
              ...
              className={cn(
                ...
                notif.type === 'critical' ? "bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-900/50" :
                notif.type === 'warning' ? "bg-orange-50 dark:bg-orange-950/30 border-orange-200 dark:border-orange-900/50" :
            ...
            >
              ...
              >
                <div className={cn(
                  ...
                  notif.type === 'critical' ? "bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-200" :
                  notif.type === 'warning' ? "bg-orange-100 dark:bg-orange-900/50 text-orange-600 dark:text-orange-200" :
                  "bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-200"
                )}>
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h3 className={cn(
                      "font-bold text-base",
                      notif.type === 'critical' ? "text-red-900 dark:text-red-100" :
                      notif.type === 'warning' ? "text-orange-900 dark:text-orange-100" :
                      "text-blue-900 dark:text-blue-100"
                    )}>{notif.title}</h3>
                    <span className={cn(
                      "text-[10px] font-medium opacity-70",
                      notif.type === 'critical' ? "text-red-800 dark:text-red-200" :
                      notif.type === 'warning' ? "text-orange-800 dark:text-orange-200" :
                      "text-blue-800 dark:text-blue-200"
                    )}>{notif.time}</span>
                  </div>
                  <p className={cn(
                    "text-sm mt-1 leading-snug opacity-80",
                    notif.type === 'critical' ? "text-red-800 dark:text-red-200" :
                    notif.type === 'warning' ? "text-orange-800 dark:text-orange-200" :
                    "text-blue-800 dark:text-blue-200"
                  )}>{notif.message}</p>
                ...
                <div className="flex items-center justify-center">
                  {expandedId === notif.id ? 
                    <ChevronUp className={cn("w-4 h-4 opacity-50", notif.type === 'critical' ? "text-red-900 dark:text-red-100" : "text-foreground")} /> : 
                    <ChevronDown className={cn("w-4 h-4 opacity-50", notif.type === 'critical' ? "text-red-900 dark:text-red-100" : "text-foreground")} />
                  }
                </div>
              </div>
              ...
              <AnimatePresence>
                {expandedId === notif.id && (
                  <motion.div 
                    ...
                    className="bg-background/40 px-5 pb-5 pt-0 text-sm"
                  >
                        ...
                        <div>
                          ...
                          <ul className="space-y-2">
                            {notif.action.map((act, i) => (
                              <li 
                                key={i} 
                                className="flex items-center gap-2 text-muted-foreground p-2 rounded-lg hover:bg-background/60 cursor-pointer transition-colors border border-transparent hover:border-border active:scale-[0.98]"
                                onClick={() => handleAction(act)}
                              >
                                <div className="w-1.5 h-1.5 rounded-full bg-primary" /> 
                                <span className="flex-1 text-foreground">{act}</span>
